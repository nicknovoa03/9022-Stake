...
export let Pool2ContractAddress: `0x${string}` = '0xb277B81694369CB690Ad58FbFA68DcFfC80F9d20';
...
export const Pool1PreparedContractApprove = (props: erc20ContractAddressApproveProps) => {
  const { config } = usePrepareContractWrite({
    ...
    args: [Pool2ContractAddress!, props.tokenAmount]
  });
  return config;
};
...
export const ERC20Allowance = (props: AllowanceBalanceProps) => {
  const { data } = useContractRead({
    ...
    args: [props.ownerAddress!, Pool2ContractAddress]
  });
  return data;
};
